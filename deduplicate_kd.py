#!/usr/bin/env python3
"""
Deduplicate Critical Decisions (KD) entries in Notion database.

This script removes duplicate KD entries by KD Number, keeping the best version
...
Usage:
    python deduplicate_kd.py --dry-run          # Preview what will be done
    python deduplicate_kd.py --dry-run --auto   # Preview with auto-selection
    python deduplicate_kd.py --auto             # Execute with auto-selection
    python deduplicate_kd.py                    # Interactive mode
...
class KDDeduplicator:
    ...
    def fetch_all_kd_entries(self):
        """Fetch all active entries from KD database."""
    ...
    def find_duplicates(self):
        ...
        unique_count = len(kd_groups) - len(self.duplicates)
    ...
    def select_best_entries(self):
        ...
        for kd_number, entries in self.duplicates.items():
            ...
            if self.auto_select:
                ...
                print(f"  {kd_number}: Auto-selected entry with score {best_score}")
    ...
    def save_backup(self):
        ...
        filename = f'kd_backup_{timestamp}.json'
    ...
    def archive_duplicates(self):
        ...
        if self.dry_run:
            ...
            for entry in self.to_archive:
                print(f"  - {entry['kd_number']}: {entry['title'][:50]}...")
            return True
        ...
        for entry in self.to_archive:
            try:
                ...
                print(f"  ✅ Archived: {entry['kd_number']}")
                success_count += 1
            except Exception as e:
                print(f"  ❌ Failed: {entry['kd_number']} - {e}")
                fail_count += 1
...
def main():
    parser = argparse.ArgumentParser(description='Deduplicate KD entries in Notion')
    parser.add_argument('--dry-run', action='store_true', help='Preview without making changes')
    parser.add_argument('--auto', action='store_true', help='Automatically select best entries')
    args = parser.parse_args()

    print("="*70)
    print("  Critical Decisions (KD) Deduplication")
    print("="*70)

    if args.dry_run:
        print("\n🔍 DRY RUN MODE - No changes will be made")

    deduper = KDDeduplicator(dry_run=args.dry_run, auto_select=args.auto)

    # Fetch all entries
    if not deduper.fetch_all_kd_entries():
        return
    ...
    if deduper.archive_duplicates():
        ...
        if not args.dry_run:
            print(f"\n💡 To restore: python restore_kd_from_backup.py {backup_file}")
    else:
