# Setup Local Supabase Anon Key Storage
# Henter nøkkel fra Secret Manager og lagrer lokalt i Windows Credential Manager

Write-Host ""
Write-Host "=== Setup Lokal Supabase Anon Key ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Henter Supabase Anon Key fra Secret Manager..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "   .\scripts\save_supabase_anon_key_to_secret_manager.ps1" -ForegroundColor Gray
    exit 1
}

# Håndter array output fra gcloud
if ($output -is [array]) {
    $anonKey = $output -join "`n"
} else {
    $anonKey = $output.ToString()
}
...
if ($anonKey.Length -ge 10) {
    Write-Host "Nøkkel hentet (starter med: $($anonKey.Substring(0,10))...)" -ForegroundColor Green
} else {
    Write-Host "Nøkkel hentet (lengde: $($anonKey.Length) tegn)" -ForegroundColor Green
}

# Slett eksisterende credential hvis den finnes
Write-Host ""
Write-Host "Sletter eksisterende credential (hvis finnes)..." -ForegroundColor Gray
cmdkey /delete:SupabaseAPI:anon-key 2>$null | Out-Null

# Lagre i Windows Credential Manager
Write-Host "Lagrer nøkkel i Windows Credential Manager..." -ForegroundColor Cyan
$result = & cmdkey /generic:SupabaseAPI:anon-key /user:supabase-anon /pass:$anonKey 2>&1

if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "Supabase Anon Key lagret lokalt!" -ForegroundColor Green
    Write-Host ""
