...
} from '/var/task/navlosen/frontend/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '/var/task/navlosen/frontend/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/navlosen/frontend')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/navlosen/frontend',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/navlosen/frontend/.netlify/dist/run/handlers/server.js')
